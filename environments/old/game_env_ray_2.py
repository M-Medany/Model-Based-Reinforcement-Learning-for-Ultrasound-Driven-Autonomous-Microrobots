...
class MicrorobotEnv(gym.Env):
    ...
    def __init__(self, render_mode: str="human", microbubble_radius: int=5, 
        ...
        print("microbubble_radius: ", self.microbubble_radius)
        ...
        self.obstacles = pygame.transform.scale(self.obstacles, (64, 64))
        self.num_envs = 1
        ...
        #self.action_space = spaces.Discrete(n=4, start=1)
        #self.action_space = spaces.Discrete(n=5, start=0)
        ...
        # self.tolerance_target_reached = self.microbubble_radius*ratio
        ...
        if self.reward_function == 'linear':
            ...
            self.reward_function = lambda x: self.reward_step*(1/(x+0.1)) + const
        elif self.reward_function == 'inverse_squared':
            self.reward_function = lambda x: self.reward_step*(1/(x**2+0.1)) + const
        elif self.reward_function == 'binary':
            self.reward_function = lambda x: self.reward_step*0*x
        else:
            raise Exception("Reward function not implemented")
        self.agent_locations = [self.agent_location.copy()]
    ...
    def reset(self, *, seed=None, options=None):
        ...
        if self.collision:
            for _ in range(self.config['Action_space_settings']['SAFE_STEPS']):
                    ...
                    direction = self._action_to_direction[int(safe_piezo-1)]
                    self.agent_location = self.agent_location + direction
            self.target_location = self.find_legal_point_target_close(self.obstacles, self.agent_location/64)
    ...
    # def find_legal_point_target_close(self, binary_image: np.ndarray, start_point: np.ndarray=None):
    ...
    #     location = np.array([location_y*64, location_x*64], dtype=int)
    ...
    #     return self.find_legal_point_target_close(binary_image, start_point)
    ...
    def _get_obs(self):
        ...
        agent_pos = self.get_agent_pos()/64
        target_pos = self.get_target_location()/64
        ...
        return np.array((*agent_pos, *target_pos), dtype=np.float32)
        return img_rescaled
    ...
    # def check_collisions(self, player_location_x: int, player_location_y: int):
    #     collisions = []
    #     for i in range(player_location_x - self.microbubble_radius, player_location_x + self.microbubble_radius):
    #         for j in range(player_location_y - self.microbubble_radius, player_location_y + self.microbubble_radius):
    #             pixel_color = self.obstacles.get_at((i, j))
    #             if pixel_color == (0, 0, 0, 255):  # Assuming black pixels have a value of 0 pixel_color = self.obstacles.get_at((x, y))
    #                 if i > player_location_x:
    #                     collisions.append(PIEZO_DIRECTIONS.RIGHT)
    ...
    def _get_safe_direction_from_img(self, cluster_center):
        ...
        if directions == []:
            ...
            #return np.random.choice([PIEZO_DIRECTIONS.RIGHT, PIEZO_DIRECTIONS.UP, PIEZO_DIRECTIONS.DOWN, PIEZO_DIRECTIONS.LEFT])
...
class MicrorobotEnvGameRayWrapped2(MicrorobotEnv):
...
class MicrorobotEnvGameRayWrappedMaS2(MicrorobotEnv):
