...
class MicrorobotEnvTest(gym.Env):
    metadata = {"render_modes": ["human", "rgb_array"], "render_fps": 90, "reward_types": ["sparse", "dense"]}  

    def __init__(self, render_mode: str=" ", microbubble_radius: int=5, image=None, timeout: int=100, subepisode_sampling: bool=False): #TODO: we could include reward type. timeout in config?, include timeout in code
        ...
        # self.observation_space = spaces.Dict({
        # 'image': spaces.Box(low=0, high=255, shape=(self.config['Layout_settings']['IMG_SIZE'], self.config['Layout_settings']['IMG_SIZE'], 3), dtype=np.uint8), #Check that this is correct, maybe low=0, high=255?, also check colour channels
        # # 'target_location': spaces.Box(low=0, high=self.window_size-1, shape=(2,), dtype=int),
        # # 'robot_location': spaces.Box(low=0, high=self.window_size-1, shape=(2,), dtype=int)
        # })
        self.observation_space = spaces.Box(low=0, high=255, shape=(self.config['Layout_settings']['IMG_SIZE'], self.config['Layout_settings']['IMG_SIZE'], 3), dtype=np.float32)
    ...
    def _get_obs(self):
        ...
        return img #{'image': img, 'agent_position': agent_pos, 'target_position': target_pos}
...
class MicrorobotEnvTestRay(MicrorobotEnvTest):
    def __init__(self, env_config):
        super().__init__(render_mode=env_config["render_mode"], microbubble_radius=env_config["microbubble_radius"], image=env_config["image"], timeout=env_config["timeout"])