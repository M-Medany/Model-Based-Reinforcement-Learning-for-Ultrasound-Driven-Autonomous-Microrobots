...
class MicrorobotEnv(gym.Env):
    ...
    def check_collisions(self, player_location_x: int, player_location_y: int):
        collisions = []
        for i in range(int(player_location_x) - self.collision_reset_tolerance, int(player_location_x) + self.collision_reset_tolerance):
            for j in range(int(player_location_y) - self.collision_reset_tolerance, int(player_location_y) + self.collision_reset_tolerance):
                if i < 64 and i > 0:
                    if j < 64 and j > 0:
